...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  // malloc a board_t structure
  board_t * b = malloc(sizeof(*b));
  b->width = w;
  b->height = h;
  b->totalMines = numMines;
  // malloc an array of int* for row pointers
  b->board = malloc(h * sizeof(b->board));
  // malloc an array to hold 2D board data
  int * board_data_ptr = malloc(w * h * sizeof(b->board));
  // calculate and fill the addresses of each row
  for (int i = 0; i < h; i++)
    b->board[i] = board_data_ptr + i * w;
  // initialize the board with UNKNOWN
  for (int i = 0; i < h * w; i++)
    board_data_ptr[i] = UNKNOWN;
  // add mines
